...
export default class AnimationManager extends EventListener {
  constructor(canvasRef, animBarRef) {
    ...
    this.skipBackButton = addControlToAnimationBar(
      animBarRef,
      "Button",
      "Skip Back",
      () => this.skipBack()
    );
    this.stepBackButton = addControlToAnimationBar(
      animBarRef,
      "Button",
      "Step Back",
      () => {
    ...
    this.playPauseBackButton = addControlToAnimationBar(
      animBarRef,
      "Button",
      "Pause",
      () => this.doPlayPause()
  ...
  finishUndoBlock(undoBlock) {
    ...
    if (this.undoAnimationStepIndices.length === 0) {
      ...
      if (this.undoAnimationStepIndicesStack.length > 0) {
        this.undoAnimationStepIndices =
          this.undoAnimationStepIndicesStack.pop();
      }
    ...
    if (this.currentlyAnimating) {
      ...
      for (i = 0; i < this.currentBlock.length; i++) {
        if (
          ...
          const progress = this.currFrame / this.animationBlockLength;
          ...
          const newX = this.lerp(
            this.currentBlock[i].fromX,
            this.currentBlock[i].toX,
            easedProgress
          );
          const newY = this.lerp(
            this.currentBlock[i].fromY,
            this.currentBlock[i].toY,
            easedProgress
          );
  ...
  startTimer() {
    this.timer = setTimeout(() => this.timeout(), 16);
  }
...
export const act = {
  ...
  createRectangle(params) {
    ...
    params[8] = params[8] || "#1e293b";
    params[9] = params[9] || "#e2e8f0";
  ...
  createLabel(params) {
    // id, label | x, y, centered, isPointer
    params[2] = params[2] || 0;
    params[3] = params[3] || 0;
    params[4] = params[4] !== false && params[4] !== 0;
  ...
  createLinkedListNode(params) {
    ...
    params[9] = params[9] || "#1e293b";
    params[10] = params[10] || "#e2e8f0";
  ...
  createDoublyLinkedListNode(params) {
    ...
    params[5] = params[5] || 0;
    params[6] = params[6] === undefined ? 0.25 : params[6];
    params[7] = params[7] || "#1e293b";
    params[8] = params[8] || "#e2e8f0";
  ...
  setEdgeColor(params) {
    ...
    const oldColor = this.animatedObjects.setEdgeColor(
      params[0],
      params[1],
      params[2]
    );
  ...
  setEdgeAlpha(params) {
    // fromID, toID, alpha
    const oldAlpha = this.animatedObjects.setEdgeAlpha(
      params[0],
      params[1],
      params[2]
    );
  ...
  setEdgeHighlight(params) {
    ...
    this.undoBlock.push(
      new UndoHighlightEdge(params[0], params[1], oldHighlight)
    );
  },
  setEdgeThickness(params) {
    // fromID, toID, thickness
    const oldThickness = this.animatedObjects.setEdgeThickness(
      params[0],
      params[1],
      params[2]
    );
    this.undoBlock.push(
      new UndoSetEdgeThickness(params[0], params[1], oldThickness)
    );
  },
  setRectangleEdgeThickness(params) {
    ...
    this.undoBlock.push(
      new UndoSetRectangleEdgeThickness(params[0], oldThicknessArray)
    );
  },
  setHeight(params) {
    // id, height
