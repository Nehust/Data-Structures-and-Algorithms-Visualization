...
const ARRAY_INITIAL_X = 65;

const ARRAY_Y_POS = 95;	
const ARRAY_LABEL_Y_POS = 130;

const ARRAY_RESIZE_Y_POS = 180;
const ARRAY_RESIZE_LABEL_Y_POS = 210;

const HEAP_X_POSITIONS = [
  0, 450, 250, 650, 150, 350, 550, 750, 100, 200, 300, 400, 500, 600, 700, 800,
  75, 125, 175, 225, 275, 325, 375, 425, 475, 525, 575, 625, 675, 725, 775, 825,
];

const HEAP_Y_POSITIONS = [
  0, 200, 270, 270, 340, 340, 340, 340, 410, 410, 410, 410, 410, 410, 410, 410,
...
export default class Heap extends Algorithm {
  ...
  addControls() {
    ...
    this.buildHeapButton = addControlToAlgorithmBar(
      "Button",
      "BuildHeap",
      horizontalGroup
    );
  ...
  createArray() {
    ...
    for (let i = 0; i < this.array_size; i++) {
      ...
      this.cmd(act.setForegroundColor, this.arrayLabels[i], "#000000ff");
    }
  ...
  setup() {
    ...
    this.createArray();
    this.cmd(act.createLabel, this.descriptLabel1, "", 60, 40, 0);
    this.cmd(act.createLabel, this.descriptLabel3, "", 400, 40, 0);
  ...
  randomCallback() {
    ...
    for (let i = 0; i < RANDOM_ARRAY_SIZE; i++) {
      values += (
        Math.floor(Math.random() * (MAX_DATA_VALUE - MIN_DATA_VALUE)) +
        MIN_DATA_VALUE
      ).toString();
    ...
    while (index * 2 <= this.currentHeapSize) {
      ...
      if (index * 2 + 1 <= this.currentHeapSize) {
        this.cmd(
          act.setText,
          this.descriptLabel3,
          `Finding ${this.order} of two children`
        );
  ...
  remove() {
    ...
    this.cmd(act.move, this.descriptLabel2, 60, 45);
    this.cmd(act.step);
  ...
  buildHeap(list) {
    ...
    if (!list.length) {
      ...
      list.map(Number).filter((x) => x > 999 || Number.isNaN(x)).length
    ) {
      this.cmd(
        act.setText,
        this.infoLabelID,
    ...
    for (let i = 1; i <= this.currentHeapSize; i++) {
        ...
        this.cmd(act.setTextColor, this.circleObjs[i], "#000000");
    ...
    this.cmd(
      act.setText,
      this.descriptLabel1,
      "Enforcing order property using downheap"
    );
  ...
  insertElement(insertedValue) {
    ...
    this.cmd(
      act.setText,
      this.descriptLabel1,
      "Enqueueing Element: " + insertedValue
    );
    this.cmd(act.step);
    ...
    this.cmd(act.setTextColor, this.circleObjs[this.currentHeapSize], "#000000");

    this.cmd(act.createLabel, this.descriptLabel2, insertedValue, 60, 45, 1);
    this.cmd(act.setTextColor, this.descriptLabel2, "#000000");
    if (this.currentHeapSize > 1) {
      this.cmd(
        act.connect,
  ...
  resize(size, add) {
    ...
    if (add) {
      for (let i = 0; i < this.array_size; i++) {
        ...
        this.cmd(act.setForegroundColor, this.newArrayLabels[i], "#f6f6f6ff");
      }
      this.cmd(act.step);
