...
const DFS_STACK_TOP_COLOR = "#4bd741ff";
const VISITED_COLOR = "#99CCFF";

const INFO_MSG_X = 60;
const INFO_MSG_Y = 15;

const LIST_START_X = 60;
const LIST_START_Y = 70;
const LIST_SPACING = 25;

const VISITED_START_X = 60;
const VISITED_START_Y = 125;

const CURRENT_VERTEX_LABEL_X = 60;
const CURRENT_VERTEX_LABEL_Y = 150;
const CURRENT_VERTEX_X = 200;
const CURRENT_VERTEX_Y = 160;

const STACK_LABEL_X = 60;
const STACK_LABEL_Y = 190;

const STACK_START_X = 120;
const SMALL_STACK_START_Y = 335;
const LARGE_STACK_START_Y = 465;
const SMALL_STACK_SPACING = 20;
const LARGE_STACK_SPACING = 16;

const RECURSION_START_X = 200;
const RECURSION_START_Y = 215; // Shifted down to avoid overlapping with stack label if recursive
...
export default class DFS extends Graph {
  ...
  setup(adjMatrix) {
    ...
    this.animationManager.setAllLayers([0, 1, this.currentLayer]);
  ...
  doDFSStack(startValue) {
    ...
    if (vertex < 0 || vertex >= this.size) {
      ...
      this.cmd(
        act.setText,
        this.infoLabelID,
        startValue + " is not a vertex in the graph"
      );
    ...
    this.stackSpacing = this.isLarge
      ? LARGE_STACK_SPACING
      : SMALL_STACK_SPACING;
  ...
  doDFSRecursive(startValue) {
    ...
    this.cmd(
      act.setText,
      this.infoLabelID,
      "About to recurse to " + this.toStr(startValue)
    );
    this.cmd(act.step);
    ...
    this.cmd(
      act.setText,
      this.infoLabelID,
      "Returned from " + this.toStr(vertex) + ", done"
    );
