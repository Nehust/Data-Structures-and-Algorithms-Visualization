...
// Queue duyệt cây
const QUEUE_START_X = 650;
const QUEUE_START_Y = 20;
...
export default class BST extends Algorithm {
  ...
  addH(data, curr) {
    ...
    if (this.compare(data, curr.data) < 0) {
      this.cmd(
        act.setText,
        0,
        `${data} < ${curr.data}. Looking at left subtree`
      );
      this.cmd(act.step);
      ...
      this.cmd(
        act.setText,
        0,
        `${data} > ${curr.data}. Looking at right subtree`
      );
      this.cmd(act.step);
  ...
  remove(data) {
    ...
    const firstLabel = this.nextIndex;
    this.predSuccMethod =
      this.predSucc === "succ" ? "remove_successor" : "remove_predecessor";
    for (let i = firstLabel; i < this.nextIndex; i++) this.toClear.push(i);
      ...
      if (curr.left == null && curr.right == null) {
        ...
        if (this.predSucc === "succ") {
          this.cmd(
            act.setText,
            0,
            `Two-child case, replace data with successor`
          );
          this.cmd(act.step);
          ...
          this.cmd(
            act.setText,
            0,
            `Two-child case, replace data with predecessor`
          );
          this.cmd(act.step);
  ...
  removeSucc(curr, dummy) {
    this.cmd(act.setHighlight, curr.graphicID, 1, "#ff0000ff");
    this.cmd(act.step);
  ...
  removePred(curr, dummy) {
    this.cmd(act.setHighlight, curr.graphicID, 1, "#f31212ff");
    this.cmd(act.step);
